...
def AcceptReject_x(scale=1,c1=15/(8),power=5): #random sample functions
    while True:
...
#(-(2*(x-thetax))**9/9+4*(2*(x-thetax))**7/7-6*(2*(x-thetax))**5/5+4*(2*(x-thetax))**3/3-2*(x-thetax)+128/315)
# In[122]:
...
reg_Tcum.reshape(1,10)
reg_Tcum_tt.reshape(1,10)
reg_Tcum_tf.reshape(1,10)
reg_Tcum_bt.reshape(1,10)
for T in T_l:
    ...
    for times in range(10):
        ...
        for k in range(epi_num):
         ...
         #   X=X.reshape(1,5)
            while t<=l_k:
                if t<=l_k1:  
                    x=np.sqrt(2/3)*np.array([AcceptReject_x(1,15/8,5) for _ in range(3)])
