...
l_0=200
#x_m0=np.zeros((100,3))
#for i in range(1,len(x_m0)):
#    x_m0[i]=0.2*x_m0[i-1]+np.array(AcceptReject_2(8,half)).reshape(1,3)
         ...
         #   X=X.reshape(1,5)
            while t<=l_k:
                if t<=l_k1:  
                    ...
                    #print(x)#np.sqrt(2/3)*np.array([AcceptReject_x(1,15/8,5) for _ in range(3)])
                ...
                #ker=Kernel(v_i,y_t,l_k1) 
                    x=np.dot(mat1,x_m.T)+np.dot(mat2,x_m1.T)+np.array(AcceptReject_2(6,half))#np.sqrt(2/3)*np.array([AcceptReject_x(1,15/8,5) for _ in range(3)])
                    #print(x)
                    x_m1=x_m
                    x_m=x
                    theta_x=3+np.dot(theta,x)
                   ...
                   # reg.append(abs(est_price-op_price)**2)#exploitation
                   # reg_tt.append(abs(est_price_tt-op_price)**2)
                   # reg_tf.append(abs(est_price_tf-op_price)**2)
                   # reg_bt.append(0)
                    np.savetxt("reg1.csv", reg, delimiter=",")
                    np.savetxt("regtt.csv", reg_tt, delimiter=",")
                    np.savetxt("regtf.csv", reg_tf, delimiter=",")
