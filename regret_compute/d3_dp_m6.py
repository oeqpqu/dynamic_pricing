...
def AcceptReject_2(b=8,cov=half): #random sample functions
    while True:
...
class Solution:
    ...
    def phi_root(self,c,d,thetax):
        ...
        m_n=(a_n+b_n)/2
        iter=0
        while abs(self.phi(m_n,thetax)) >= self.th and iter<=5000:  # 判断是否收敛
    ...
    def phi(self, x, thetax):
        ...
        return x-(-(2*x)**13/13+6*(2*x)**11/11-15*(2*x)**9/9+20*(2*x)**7/7-3*(2*x)**5+2*(2*x)**3-2*x+0.34099234)/(2*(1-(2*x)**2)**6)+thetax    
...
for T in T_l:
    ...
    for times in range(30):
        ...
        for i in range(2, len(x_m0)):
            x_m0[i] = (np.dot(mat1,x_m0[i - 1].T) +(np.dot(mat2,x_m0[i-2].T))+np.array(AcceptReject_2(8, half))).reshape(1, 3)
        x_m1=x_m0[498]
        x_m=x_m0[499]
        #print(x_m)
        #print(np.dot(mat1,x_m.T))
        for k in range(epi_num):
         ...
         #   X=X.reshape(1,5)
            while t<=l_k:
                if t<=l_k1:  
                    x=np.dot(mat1,x_m.T)+np.dot(mat2,x_m1.T)+np.array(AcceptReject_2(8,half))
                    #print(x)#np.sqrt(2/3)*np.array([AcceptReject_x(1,15/8,7) for _ in range(3)])
                ...
                #ker=Kernel(v_i,y_t,l_k1) 
                    ...
                    x_m1=x_m
                    x_m=x
                    #x=np.array(AcceptReject_2(8,half)) #np.sqrt(2/3)*np.array([AcceptReject_x(1,15/8,7) for _ in range(3)])
                    theta_x=3+np.dot(theta,x)
                    #print(theta_x)
                    ...
                    #if est_price!=est_price:
                     #   print('fail')
                    #    est_price=random.uniform(-1,1)+theta_est
                    #print(est_price)
                    #if est_price==float('NaN'):
                    #    est_price=est_price_tf
                   ...
                   #if T==6300:
                        #diff.append(abs(est_price-op_price)**2-abs(est_price_tf-op_price)**2)
                    #print(abs(est_price_tt-op_price))
                    #est_price_tf=solu.phi_root(-0.97,0.97,theta_est)+theta_est
...
#np.savetext('diff.csv',diff,delimiter=",")
