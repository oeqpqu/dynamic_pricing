...
#epi_num=math.ceil(math.log(T/l_0+1, 2))
    ...
    reg_Tcum_ucb_acc=np.vstack([reg_Tcum_ucb_acc,reg_ucb_acc.reshape(1,12000)]) 
...
# In[10]:
##############################################
############Comparision with RMLP Algorithm ############
##############################################
...
for times in range(10):
    ...
    for k in range(epi_num):
        t=0
        l_k=l_0*2**k
        l_k1=math.floor((l_k*2)**(13/23)) #when m is unknown l_k1=math.floor((l_k*2)**((2*m+1)/(4*m-1))) #m is chosen via choose_m:
        l_k1_total+=l_k1
        v_i=[]
        v_i_true=[]
        v_t=[]
        ...
        while t<=l_k:
            if t<=l_k1:  
                ...
                if t==l_k1:
                    ...
                    v_i_true=np.array(v_i_true) ##When m is unknown, choose $m$ via choose_m(l_k1,v_i_explore_true,y_t_explre), the remaining procedures are similar.
    ...
    np.savetxt("reg_t1.csv", np.array(reg_Tcum_acc), delimiter=",") #output our regret 
