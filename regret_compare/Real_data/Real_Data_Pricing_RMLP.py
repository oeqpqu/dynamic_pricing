...
#data_new=data_new[1:100000,[2,3,4,5]]
...
# In[84]:
    ...
    for k in range(epi_num):
        t=0
        l_k=l_0*2**k
        l_k1=math.floor((l_k*2)**(5/7)) ##when m is unknown l_k1=math.floor((l_k*2)**((2*m+1)/(4*m-1))) #m is chosen via choose_m: Here we use Pessimistic way to choose m. List in our remark 12.
        l_k1_total+=l_k1
        v_i=[]
        v_i_true=[]
        v_t=[]
     ...
     #   X=X.reshape(1,5)
        while t<=l_k:
            ...
            #ker=Kernel(v_i,y_t,l_k1) 
                x=data_new2[t+l_0*(2**k-1),:]#np.array(AcceptReject_2(4,half))#np.sqrt(2/3)*np.array([AcceptReject_x(1,15/8,3) for _ in range(3)])
                theta_x=0.3+np.dot(theta,x)
                #print(theta_x)
